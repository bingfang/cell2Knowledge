...
with open("../data/HLCA_CellRef matching_ver3.txt", "r") as f:
    ...
    for row in data_in[2:64]:
        ...
        if "CL" in field[40] and "skos" in field[38]:
            Cell_set_name=field[39]
            Cell_type_name=field[5]
            Biomarker_combination_name=field[30]
            idx = field[0]
            
            Cell_set_key = Cell_set_name.replace(" ", "-").replace(",", "_") + "_" + idx
            print(Cell_set_key)
            Cell_type_key = Cell_type_name.replace(" ", "-").replace("//", "_") + "_" + idx
            print(Cell_type_key)
            Biomarker_combination_key = Biomarker_combination_name.replace(", ", "_")  + "_" + idx
            Biomarker_combination_key = Biomarker_combination_key.replace("'", "").replace('"', '').replace('[', '').replace(']', '')
...
"""              
            ...
            if biomarker_cellSet_edge_key not in biomarker_cellSet_list:
                ...
                Biomarker_combination_key = Biomarker_combination_dic[Biomarker_combination_name]
                Cell_set_key = Cell_set_dic[Cell_set_name]
                d4 = {"_key": biomarker_cellSet_edge_key, "_from": f"Cell_set/{Cell_set_key}","_to": f"Biomarker_combination/{Biomarker_combination_key}","name": "PART_OF"}
            ...
            if cellType_AnatomicStructure_edge_key not in cellType_AnatomicStructure_list:
                ...
                Cell_type_key = Cell_type_dic[Cell_type_name]          
                d5 = {"_key": cellType_AnatomicStructure_edge_key, "_from": "Anatomic_structure/Organ_12345","_to": f"Cell_type/{Cell_type_key}","name": "PART_OF"}
            ...
            if cellSet_cellType_edge_key not in cellSet_cellType_list:
                ...
                Cell_type_key = Cell_type_dic[Cell_type_name]
                Cell_set_key = Cell_set_dic[Cell_set_name]         
                d6 = {"_key": cellSet_cellType_edge_key, "_from": "Cell_set/{Cell_set_key}","_to": f"Cell_type/{Cell_type_key}","name": "PART_OF"}
